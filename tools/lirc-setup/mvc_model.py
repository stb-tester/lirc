...
def write_results(model, result_dir, view):
    ...
    def write_modprobe(log):
        ...
        if not hasattr(model.config, 'modprobe') or not model.config.modprobe:
            return log
    ...
    def write_options(options, log):
        ...
        if model.config.modinit:
            ...
            modinit = model.config.modinit
            options.set('modinit', 'code', modinit)
...
def check_modules(config):
    ...
    if 'modinit' not in config or not config.modinit:
        return ''
    return "Required kernel setup: " + config.modinit
...
class EventDeviceListModel(DeviceListModel):
    ...
    def __init__(self, model):
        config = model.db.find_config('id', 'devinput')
...
class LircDeviceListModel(DeviceListModel):
    ...
    def __init__(self, model):
        config = model.db.find_config('id', 'default')
        DeviceListModel.__init__(self, config)
...
class UdpPortDeviceList(DeviceListModel):
    ...
    def __init__(self, model):
        config = model.db.find_config('id', 'udp')
        DeviceListModel.__init__(self, config)
...
class AutoDeviceList(DeviceListModel):
    ...
    def __init__(self, model):
        config = model.db.find_config('id', self.AUTO_CONFIG)
        DeviceListModel.__init__(self, config)
...
def device_list_factory(driver, model):
    ...
    device_wildcard = driver['device_hint']
...
class Model(object):
    ...
    def set_config(self, config):
        ...
        if isinstance(config, str):
            config = self.db.configs[config]
        self.config.config = config
        self.set_driver(config['driver'])
        self._call_listeners()
